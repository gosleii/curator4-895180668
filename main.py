...
def anatomy(message):
    ...
    объяснения
    на
    препаратах - [Андрей Стрелков](https: // youtube.com / @ user - rq4rn6gg5o?si = K7HiwhGWqxKNls8x)\n
    Короткие
    видео
    для
    повторения
    материала - [Easy Anatomy](https: // youtube.com / @ easyanatomyapplication?si = zsgtOnSdC59BKTPZ)''')
...
def chemistry(message):
    ...
    рисунках
    и
    схемах](https: // youtube.com / @ risuem_biohimiyu?si=H1u7nEhUtw__Xyqa)
    ''')
...
def pharma(message):
    ...
    лекции
    по
    фармакологии](https: // youtube.com / @ scidrugs?si=UqLB4sSaitSVyMt1)
    ''')

@bot.message_handler(commands=['neurology'])
def neurology(message):
    bot.send_message(message.chat.id,
                     'Поможет разложить общую неврологию по полочкам - [EasyNeurology](https://youtube.com/@easyneurology1984?si=wB4vetvMZczbkL8V)')
